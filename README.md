# Ilya Nartov

Product Analyst / LLM Engineer.

Building autonomous AI-agent workflows and measurable product analytics.
Currently at **Joom** (e-commerce); previously **Yandex Market**, **T-Bank**, **HSE**.

## What I work on

- **LLM agents & structured output** — production agent loops, SGR / NextStep patterns, evaluation harnesses (OpenAI, Anthropic, Pydantic, LangGraph).
- **Product analytics & experimentation** — metric trees, A/B testing with bootstrapping, power analysis, CTR / retention studies.
- **ML for product** — embeddings-based targeting, LLM-based review scoring, LightGBM, scikit-learn.

## Pinned

| # | Repo | What it does |
|---|------|-------------|
| 1 | [PAC_cortex](https://github.com/Ilyuha888/PAC_cortex) | Autonomous LLM agent — 43/43 on the BitGN production benchmark. SGR loop, pre-flight assembler, injection-scanning pipeline. |
| 2 | [Edtech_complex_analysis](https://github.com/Ilyuha888/Edtech_complex_analysis) | A/B test of a new payment mechanic with bootstrap + ClickHouse metric automation. |
| 3 | [Game_Addiction](https://github.com/Ilyuha888/Game_Addiction) | Bayesian vs frequentist ANOVA, LCA segmentation, power analysis on a 1%-of-population dataset. |
| 4 | [E-learning](https://github.com/Ilyuha888/E-learning) | Product analytics over 2 years of platform data: metric design, RFM segmentation. |
| 5 | [Labor-questionarie_processing](https://github.com/Ilyuha888/Labor-questionarie_processing) | Psychometric questionnaire validation (EFA / CFA / ESEM) targeting Q1/Q2 journals. |
| 6 | [ab_library](https://github.com/Ilyuha888/ab_library) | Small Python helpers for A/B significance & power calculations. |

## Stack

![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=white&style=flat)
![SQL](https://img.shields.io/badge/-SQL-4479A1?logo=postgresql&logoColor=white&style=flat)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?logo=postgresql&logoColor=white&style=flat)
![ClickHouse](https://img.shields.io/badge/-ClickHouse-FFCC01?logo=clickhouse&logoColor=black&style=flat)
![BigQuery](https://img.shields.io/badge/-BigQuery-4285F4?logo=googlebigquery&logoColor=white&style=flat)
![Spark](https://img.shields.io/badge/-Spark-E25A1C?logo=apachespark&logoColor=white&style=flat)
![Airflow](https://img.shields.io/badge/-Airflow-017CEE?logo=apacheairflow&logoColor=white&style=flat)
![Pydantic](https://img.shields.io/badge/-Pydantic-E92063?logo=pydantic&logoColor=white&style=flat)
![LangGraph](https://img.shields.io/badge/-LangGraph-1C3C3C?logo=langchain&logoColor=white&style=flat)
![OpenAI](https://img.shields.io/badge/-OpenAI-412991?logo=openai&logoColor=white&style=flat)
![Anthropic](https://img.shields.io/badge/-Anthropic-191919?logo=anthropic&logoColor=white&style=flat)
![scikit-learn](https://img.shields.io/badge/-scikit--learn-F7931E?logo=scikitlearn&logoColor=white&style=flat)
![LightGBM](https://img.shields.io/badge/-LightGBM-02569B?style=flat)
![R](https://img.shields.io/badge/-R-276DC3?logo=r&logoColor=white&style=flat)

## Contact

- Email: [nartov.nartov@gmail.com](mailto:nartov.nartov@gmail.com)
- Telegram: [@ImmatureTurtle](https://t.me/ImmatureTurtle)
- LinkedIn: *URL to be added*
- CV: available on request
